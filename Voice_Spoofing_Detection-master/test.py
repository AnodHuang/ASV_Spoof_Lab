...
def test_single_file(feat_model_path, loss_model_path, lfcc_feature_path, add_loss, device):
    ...
    with open(lfcc_feature_path, 'rb') as f:
        lfcc = pickle.load(f)


    lfcc = torch.tensor(lfcc).unsqueeze(0).unsqueeze(0).float().to(device)
    ...
    with torch.no_grad():
        ...
        if add_loss == "ocsoftmax":
            ang_isoloss, score = loss_model(feats, torch.zeros(1, dtype=torch.long).to(device))
        elif add_loss == "amsoftmax":
            outputs, moutputs = loss_model(feats, torch.zeros(1, dtype=torch.long).to(device))
            score = F.softmax(outputs, dim=1)[:, 0]

        print(f"Score: {score.item()}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test a single audio file for voice spoofing detection.")
    parser.add_argument('-m', '--model_dir', type=str, help="Path to the trained model",
                        default="/home/chenzhieg/Voice_Spoofing_Detection-master/models1028/ocsoftmax/test_results")
    parser.add_argument('-f', '--feature_path', type=str,
                        help="Path to the LFCC feature file", default="/home/chenzhieg/Voice_Spoofing_Detection-master/LFCC_1111.pkl")
    parser.add_argument('-l', '--loss', type=str, default="ocsoftmax",
                        choices=["softmax", 'amsoftmax', 'ocsoftmax'], help="Loss function")
    parser.add_argument("--gpu", type=str, help="GPU index", default="0")
    args = parser.parse_args()
    ...
    # 调用测试函数
    test_single_file(model_path, loss_model_path, args.feature_path, args.loss, args.device)
